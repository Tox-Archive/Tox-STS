...
It is recommended that the tox data file, ``tox_save``, be located on both the running directory, for portable use of the Tox client, and ``~/.config/tox/`` (Linux) or ``%APPDATA%/tox/`` (Windows), for persistent installations.
